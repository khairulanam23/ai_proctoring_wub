...
def test_consecutive_detections_consolidated_into_single_event() -> None:
    ...
    assert "cell phone" in event.reason.lower()


def test_object_presence_event_reason_field() -> None:
    """Test ObjectPresenceEvent reason field creation and serialization."""
    ev = ObjectPresenceEvent(
        event_id="obj_event_001",
        object_class="cell phone",
        start_seconds=7.5,
        end_seconds=10.0,
        duration_seconds=2.5,
        formatted_start="00:07.500",
        formatted_end="00:10.000",
        detection_count=6,
        max_confidence=0.91,
        average_confidence=0.88,
        is_duration_qualified=True,
    )
    assert ev.reason == "Detected object class: cell phone"
    data = ev.to_dict()
    assert "reason" in data
    assert data["reason"] == "Detected object class: cell phone"


def test_person_count_change_event_reason_field() -> None:
    """Test PersonCountChangeEvent reason field creation and serialization."""
    chg = PersonCountChangeEvent(
    ...
    assert "1 to 2" in chg.reason
    data = chg.to_dict()
    assert "reason" in data
    assert "1 to 2" in data["reason"]
...
def test_absence_tolerance_bridging() -> None:
    ...
    assert "cell phone" in report.events[0].reason
...
def test_minimum_event_duration_qualification() -> None:
    ...
    assert "book" in report.events[0].reason
...
def test_multiple_simultaneous_object_classes() -> None:
    ...
    engine.ingest_frame(
        ...
        45,
        1,
        [make_detected_obj("person"), make_detected_obj("cell phone"), make_detected_obj("laptop")],
    )
    engine.ingest_frame(
        2.0,
        60,
    ...
    assert "cell phone" in phone_event.reason
...
def test_temporal_event_report_json_serialization() -> None:
    ...
    engine.ingest_frame(
        1.0, 30, 2, [make_detected_obj("person", 0.92), make_detected_obj("cell phone", 0.88)]
    )
    report = engine.finalize(final_timestamp=1.5)
...
def test_render_temporal_gantt_chart(tmp_path: Path) -> None:
    ...
    result_path = render_temporal_gantt_chart(
        report, total_duration_seconds=3.0, output_path=out_file
    )
