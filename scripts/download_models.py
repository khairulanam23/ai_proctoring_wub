...
def download_file(urls: list[str], dest_path: Path, min_size: int) -> bool:
    ...
    if dest_path.exists() and dest_path.stat().st_size >= min_size:
        print(f"✓ {dest_path.name} already exists and is valid ({dest_path.stat().st_size:,} bytes).")
        return True
    ...
    for url in urls:
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (AI Proctoring Downloader)"})
            ...
            if size < min_size:
                print(f"  Warning: Downloaded file size too small ({size} bytes). Likely LFS pointer or HTML error. Trying next mirror...")
            ...
            with open(temp_path, "rb") as f:
                header = f.read(100)
                if b"version https://git-lfs" in header or b"<!DOCTYPE html>" in header or b"<html" in header:
