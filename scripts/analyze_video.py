#!/usr/bin/env python3
"""CLI script for video face presence and state transition tracking."""
...
from src.face import FaceDetector, FacePresenceAnalyzer, PresenceEvent, VideoPresenceAnalyzer


def handle_event(event: PresenceEvent) -> None:
    """Print event transition to terminal in human-readable format."""
    print(f"\n{event.timestamp_formatted} (Frame {event.frame_index})")
    print(f"Event:          {event.event_type}")
    if event.previous_status:
        print(f"Transition:     {event.previous_status} -> {event.current_status}")
    print(f"Faces detected: {event.face_count}")
