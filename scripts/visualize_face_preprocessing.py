...
def create_preprocessing_visualization(
    ...
    print(f"Resolution:     {q.min_dimension}px >= {preprocessor.min_face_size}px (Valid={q.is_size_valid})")
    if pose:
        print(f"Pose Est:       Yaw Ratio={pose.yaw_ratio:.3f}, Roll={pose.roll_angle_deg:.1f}°, Pitch Ratio={pose.pitch_ratio:.3f}")
    ...
    for name, (lx, ly) in zip(landmark_names, face.landmarks):
    ...
    def fit_into_panel(src_img: np.ndarray, ph: int = 300, pw: int = 260, title: str = "", subtitle: str = "") -> np.ndarray:
        ...
        if subtitle:
            cv2.putText(panel, subtitle, (10, ph - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200, 200, 200), 1)
        return panel
    ...
    for (lx, ly), color in zip(face.landmarks, colors):
        cv2.circle(detected_vis, (int(lx), int(ly)), 4, color, -1)
        cv2.circle(detected_vis, (int(lx), int(ly)), 5, (255, 255, 255), 1)
    sub2 = f"Yaw:{pose.yaw_ratio:.2f} Roll:{pose.roll_angle_deg:.0f}°" if pose else f"Conf: {face.confidence:.2f}"
    ...
    panel4 = fit_into_panel(result.aligned_face, target_h, target_w, "4. 5-Pt Aligned", "112x112 Canonical")

    # Panel 5: Final SFace Input
    panel5 = fit_into_panel(result.normalized_face, target_h, target_w, "5. SFace Input", "128-d Feature Ready")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize face preprocessing, landmark alignment, and pose estimation.")
    parser.add_argument(
        "image_path",
        type=str,
        nargs="?",
