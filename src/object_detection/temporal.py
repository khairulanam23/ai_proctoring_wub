...
from src.object_detection.sampling import format_timestamp
...
def render_temporal_gantt_chart(
    ...
    for event in report.events:
        ...
        width_x = max(0.08, event.duration_seconds)  # visual width floor for zero-length single-frame points
        ...
        # Label duration if wide enough
        dur_label = f"{event.duration_seconds:.1f}s ({int(event.max_confidence * 100)}%)"
        ...
        ax_events.text(
            ...
            fontsize=8.5,
            fontweight="bold",
            color="white" if event.object_class.lower() != "keyboard" else "black",
        )
    ...
    ax_person.set_title("Person Count State Transitions", fontsize=11, fontweight="bold", pad=8)
    ...
    for chg in report.person_count_changes:
        ...
        ax_person.annotate(
            f"{chg.previous_count} → {chg.new_count}",
            ...
            fontsize=8.5,
            fontweight="bold",
            bbox=dict(boxstyle="round,pad=0.2", fc="#fff9e6", ec="#d4ac0d", lw=1),
        )

    ax_person.set_xlim(0, dur)
    ax_person.set_ylim(-0.2, max(3.5, max(count_pts) + 0.8 if count_pts else 3.0))
