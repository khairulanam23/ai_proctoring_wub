...
class ObjectRelevanceFilter:
    ...
    def __init__(
        self,
        relevant_classes: Optional[Set[str]] = None,
    ...
    def visualize_report(
        ...
        if show_all and report.ignored_objects:
            for obj in report.ignored_objects:
                ...
                cv2.rectangle(vis_img, (x1, label_y1), (min(vis_img.shape[1], x1 + tw + 4), y1), (80, 80, 80), -1)
                cv2.putText(vis_img, label, (x1 + 2, y1 - 3), font, font_scale, (200, 200, 200), 1, cv2.LINE_AA)
        ...
        cv2.putText(bar, status_text, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (255, 255, 255), 1, cv2.LINE_AA)
        vis_with_bar = np.vstack([bar, vis_img])
