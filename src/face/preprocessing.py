...
from src.face.detector import FaceDetector, FaceDetection, DetectionResult
...
class PreprocessingResult:
    ...
    aligned_face: Optional[np.ndarray]  # (112, 112, 3) BGR aligned
    normalized_face: Optional[np.ndarray]  # (112, 112, 3) BGR preprocessed for SFace
    raw_face: Optional[FaceDetection]
    face_count: int
    quality: Optional[QualityMetrics]
    transform_matrix: Optional[np.ndarray]  # (2, 3) Affine matrix
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "aligned_shape": list(self.aligned_face.shape) if self.aligned_face is not None else None,
...
class FacePreprocessor:
    ...
    def __init__(
        ...
        enable_illumination_norm: Optional[bool] = None,  # Backward compatibility
        ...
        if output_size == (112, 112):
            ...
            self.canonical_template = self.CANONICAL_5_POINTS_112 * np.array([scale_x, scale_y], dtype=np.float32)
    ...
    def align_face(
        ...
        transform_matrix, inliers = cv2.estimateAffinePartial2D(src_pts, self.canonical_template, method=cv2.LMEDS)
    ...
    def normalize_illumination(self, aligned_bgr: np.ndarray, mode: Optional[str] = None) -> np.ndarray:
        ...
        elif active_mode == "mild_contrast":
