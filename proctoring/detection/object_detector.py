            ...
            if boxes is not None:
                ...
                if xyxy_tensor is not None and conf_tensor is not None and cls_tensor is not None:
                    xyxy_arr = (
                        ...
                        else np.array(xyxy_tensor)
                    )
                    conf_arr = (
                        conf_tensor.cpu().numpy()
                        if hasattr(conf_tensor, "cpu")
                        else np.array(conf_tensor)
                    )
                    cls_arr = (
                        cls_tensor.cpu().numpy()
                        if hasattr(cls_tensor, "cpu")
                        else np.array(cls_tensor)
                    )

                    for xyxy, conf, cls_id in zip(xyxy_arr, conf_arr, cls_arr, strict=False):
                        cid = int(cls_id)
